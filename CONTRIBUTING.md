...
New lesson documentation must be introduced as a complete bilingual pair in the same change.

For a lesson `NN` inside a learning lab `<lab>`, create:

- the canonical English document at `<lab>/lessons/lesson-NN-learned.md`
- the Italian translation at `<lab>/lessons/lesson-NN-learned.it.md`
...
3. register `<lab>/lessons/lesson-NN-learned.md` in `canonical_documents` inside `.github/bilingual-docs.json`
4. add the lesson to both `<lab>/README.md` and `<lab>/README.it.md`
...
### Existing Laravel lesson boundary

Laravel lessons 1 through 18 remain part of the progressive legacy migration. Lessons 19 through 21 have already been migrated to canonical bilingual pairs.

Starting with Laravel lesson 22, every new Laravel lesson follows the new-lesson workflow above.

### PHP lab boundary

The PHP lab was introduced after adoption of the bilingual policy. It therefore has no initial legacy lesson range.

Every PHP lesson, starting with PHP lesson 1, must follow the new-lesson workflow above using:
