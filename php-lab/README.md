# PHP Lab

[English](README.md) | [Italiano](README.it.md)

This lab reconstructs and reproduces the PHP module of the Kleis Software Developer course.

Recovered teacher material is evidence of the course, not proof that a topic has been learned or reproduced locally.

## Recovery workflow

source evidence -> reconstruct -> understand -> reproduce -> run -> verify -> document

## Recovery checkpoint

| Lesson | Reconstruction status | Local reproduction |
| --- | --- | --- |
| PHP 1 | RECONSTRUCTED | REPRODUCED AND VERIFIED |
| PHP 2 | RECONSTRUCTED WITH EVIDENCE GAP | NOT STARTED |
| PHP 3 | RECONSTRUCTED | NOT STARTED |
| PHP 4 | RECONSTRUCTED TO AVAILABLE SNAPSHOT | NOT STARTED |

## Lessons

- [Lesson 1 — Language fundamentals and first dynamic page](lessons/lesson-01-learned.md)

## Evidence boundary

HANDOUT CONTENT != EVIDENCE OF CLASSROOM COMPLETION

TEACHER SNAPSHOT != OUR IMPLEMENTATION

Only locally reproduced and verified work is marked as completed in this lab.

## Runtime readiness

Current local baseline:

- PHP 8.3.6
- PDO available
- pdo_sqlite available
- pdo_mysql not currently available

PDO support for MySQL is not required for PHP 1. It becomes a readiness gate before reproducing the database-backed lessons.
